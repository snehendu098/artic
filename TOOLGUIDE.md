...
## Example: Complete 1inch Integration
...
export const createOneinchGetQuote = (deps: ToolDependencies) => {
  ...
  return tool(
    async ({ fromToken, toToken, amount }) => {
      ...
      await eventLogger.emit({
        type: "tools_selected",
        data: { tool: "oneinch_get_quote", args: { fromToken, toToken, amount } }
      });

      try {
        const quote = await mntAgentKit.get1inchQuote(fromToken, toToken, amount);
        ...
        await eventLogger.emit({
          type: "error",
          data: { tool: "oneinch_get_quote", error: `Failed to get 1inch quote: ${errorMsg}` }
        });
        throw error;
      }
    },
    {
      ...
      schema: z.object({
        ...
        amount: z.string().describe("Amount of tokens to swap. Human readable: e.g: 0.01, 10, 100 etc")
      })
    }
  );
};

// Transaction swap tool
  ...
  return tool(
    async ({ fromToken, toToken, amount, slippage }) => {
      ...
      try {
        ...
        await eventLogger.emit({
          type: "error",
          data: { tool: "oneinch_swap", error: `Failed to execute 1inch swap: ${errorMsg}` }
        });
        throw error;
      }
    },
...
Example Protocols: Pyth Network (7 tools, read-only oracle), 1inch (2 tools, DEX aggregator)
